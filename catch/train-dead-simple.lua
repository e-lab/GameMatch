...
local function Memory(maxMemory)
    ...
    function memory.getBatch(model, gamma, batchSize, nbActions, dataSize)
...
for game = 1, opt.epochs do
  ...
  while (isGameOver ~= true) do
      ...
      local inputs, targets = memory.getBatch(model, opt.gamma, opt.batchSize, #gameActions, 3*opt.gridSize-1)
