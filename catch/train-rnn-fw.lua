...
local function trainNetwork(model, state, inputs, targets, criterion, sgdParams)
    ...
    local function feval(x_new)
        gradParameters:zero()
        local iii = {inputs:squeeze(), table.unpack(state)}
        local out = model:forward(iii)
...
for l = 1, opt.nLayers do
  table.insert(RNNh0, torch.zeros(opt.nHidden))
  table.insert(RNNh, torch.zeros(opt.nHidden))
end
...
for game = 1, epoch do
    ...
    -- print({currentState, table.unpack(RNNh)})
