...
-- training code:
local function trainNetwork(model, state, inputs, targets, nSeq, nbActions)
    local loss = 0
...
local ppnet = nn.SpatialMaxPooling(4,4,4,4)
local function preProcess(inImage)
  local pooled = ppnet:forward(inImage[1][{{},{94,194},{9,152}}])
...
for game = 1, opt.epochs do
    ...
    while not isGameOver do
        ...
        if reward >= 1 then 
            ...
            err = err + trainNetwork(model, RNNh0Batch, inputs, targets, nSeq, nbActions)
        end
