...
config_file_path = base_path+"scenarios/simpler_basic.cfg"
...
def create_network(session, available_actions_count):
    ...
    s1_ = tf.placeholder(tf.float32, [None] + list(resolution) + [1], name="State") # input
    a_ = tf.placeholder(tf.int32, [None], name="Action") # output
    ...
    # maxpool by 4 before RNN:
    pool1 = tf.nn.max_pool(s1_, ksize=[1, 4, 4, 1], strides=[1, 4, 4, 1], padding='SAME')
